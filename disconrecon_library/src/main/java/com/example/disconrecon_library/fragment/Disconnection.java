...
import com.example.disconrecon_library.adapter.DisconAdapter;
...
public class Disconnection extends Fragment implements View.OnClickListener, DisconAdapter.OnItemClickListener, AdapterView.OnItemSelectedListener {
    private BottomNavigationView bottomNavigationView;
    private TextView tv_total, tv_disconnected, tv_remaining, tv_date, tv_calender;
...
//                    insertData();
                ...
                case INSERT_SUCCESS:
                    ...
                    getDiscondata(loginList.get(0).getMRCODE(), date);
                    break;
    ...
    private void initialize(View view) {
        ...
        toolbar.setTitle(getResources().getString(R.string.disconnection));
        ...
        tv_disconnected = view.findViewById(R.id.txt_connected);
...
//            updateTaskList();
    ...
    private void insertData() {
        for (int i = 0; i < detailsList.size(); i++) {
            ...
            repository.insertDisData(details);
        }
    }
    ...
    private void getCount(List<DisconData> detailsList) {
        ...
        tv_disconnected.setText(String.valueOf(Integer.parseInt(String.valueOf(detailsList.size())) - REMAINING));
        tv_remaining.setText(String.valueOf(REMAINING));
    }

    //-----------------------------------------------------------------------------------------------------------------------------------------
    private void getDiscondata(String MRCODE, String DATE) {
        RetroClient1 retroClient = new RetroClient1();
        RegisterAPI api = retroClient.getClient().create(RegisterAPI.class);
        api.getDisconList(MRCODE, DATE, loginList.get(0).getPASSWORD(), device_id, Andkey).enqueue(new Callback<List<DisconData>>() {
            @Override
            public void onResponse(@NonNull Call<List<DisconData>> call, @NonNull Response<List<DisconData>> response) {
                if (response.isSuccessful()) {
                    ...
                    adapter = new DisconAdapter(getActivity(), detailsList);
                    adapter.setOnItemClickListener(Disconnection.this);
    ...
    public void show_connection_update_dialog(int id, DisconData details) {
        if (id == CONNECTION_UPDATE_DIALOG) {
            ...
            tv_dis_date.setText(details.getDIS_DATE());
            ...
            update_button.setText(getActivity().getText(R.string.disconnect));
            ...
            update_button.setOnClickListener(view1 -> {
                ...
                disconUpdate(tv_cons_no.getText().toString(), date, curr_reading.getText().toString(), REMARK, comment.getText().toString());
            });
    ...
    //-----------------------------------------------------------------------------------------------------------------------------------------
    private void disconUpdate(String Acc_id, String Dis_Date, String CURREAD, String Remarks, String Comment) {
        RetroClient1 retroClient = new RetroClient1();
        RegisterAPI api = retroClient.getClient().create(RegisterAPI.class);
        api.disconUpdate(Acc_id, Dis_Date, CURREAD, Remarks, Comment, loginList.get(0).getMRCODE(), loginList.get(0).getPASSWORD(), device_id, Andkey).enqueue(new Callback<List<Result>>() {
            @Override
    ...
    private void updateTaskList() {
        repository.getDisData().observe(this, notes -> {
            assert notes != null;
            if (notes.size() > 0) {
                detailsList = notes;
                adapter = new DisconAdapter(getActivity(), notes);
                adapter.setOnItemClickListener(Disconnection.this);
                recyclerView.setLayoutManager(new LinearLayoutManager(getActivity()));
                recyclerView.setAdapter(adapter);
                handler.sendEmptyMessage(REQUEST_RESULT_SUCCESS);
